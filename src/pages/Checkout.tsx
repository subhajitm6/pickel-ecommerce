          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-mango/10">
            ...
            <form onSubmit={handleNext}>
              ...
              <div className="mt-10 flex justify-between">
                {step > 1 && (
                  <button type="button" onClick={() => setStep(step - 1)} className="font-bold text-brown/60 hover:text-brown px-6 py-3">
                    Back
                  </button>
                )}
                <button type="submit" className="ml-auto bg-brown text-cream font-black px-10 py-4 rounded-xl hover:bg-mango hover:text-brown transition-colors shadow-lg flex items-center gap-2">
                  {step === 3 ? 'Place Order' : 'Continue'} <ArrowRight size={20} />
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="space-y-4 mb-6 max-h-64 overflow-y-auto pr-2">
              {cartItems.map(item => (
                <div key={item.id} className="flex gap-4">
                  <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${item.color} flex-shrink-0 relative`}>
                    <div className="absolute -top-2 -right-2 w-6 h-6 bg-brown text-white rounded-full flex items-center justify-center text-xs font-bold">
                      {item.quantity}
                    </div>
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-brown text-sm leading-tight">{item.name}</h4>
                    <div className="text-pickle font-black mt-1">₹{(item.price * item.quantity).toFixed(2)}</div>
                  </div>
                </div>
              ))}
            </div>
