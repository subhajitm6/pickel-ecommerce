...
export function FeaturedProducts() {
  ...
  const collections = ['All', ...Array.from(new Set(products.map((p: any) => p.category?.name).filter(Boolean)))];

  const filteredProducts = activeCollection === 'All' 
    ? products 
    : products.filter((p: any) => p.category?.name === activeCollection);
  ...
  const next = () => {
    setCurrentIndex((prev) => (prev + itemsPerPage >= filteredProducts.length ? 0 : prev + 1));
  };

  const prev = () => {
    setCurrentIndex((prev) => (prev - 1 < 0 ? Math.max(0, filteredProducts.length - itemsPerPage) : prev - 1));
  };
    ...
    <section id="products" className="py-32 bg-[#FFF2C6] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-brown/70 font-medium mb-12">Taste the tradition in every bite.</p>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4 md:px-12 min-h-[500px]">
            {error && (
              <div className="col-span-full text-center text-red-500 font-bold p-8 border-2 border-red-200 rounded-2xl bg-red-50">
                Failed to load products: {error}. Is the backend running?
              </div>
            )}
            <AnimatePresence mode="popLayout">
            {filteredProducts.slice(currentIndex, currentIndex + itemsPerPage).map((product) => (
              <motion.div
                key={product.id}
                layout
              ...
              >
                ...
                <div className={`h-72 rounded-3xl mb-8 flex items-center justify-center bg-gradient-to-br from-yellow-200 to-orange-200 opacity-80 group-hover:opacity-100 transition-opacity relative overflow-hidden`}>
                  <div className="absolute inset-0 bg-white/20 group-hover:bg-transparent transition-colors" />
                  <motion.img src={product.image || 'https://via.placeholder.com/150'} alt={product.title} className="w-40 h-48 object-cover rounded-2xl shadow-lg border border-white/50 group-hover:rotate-6 transition-transform duration-500" />
                </div>
                
                <div>
                  <h3 className="text-2xl font-black text-brown mb-2">{product.title}</h3>
