            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {cartItems.length === 0 ? (
                ...
                cartItems.map((item) => (
                  ...
                  >
                    <div className={`w-20 h-20 rounded-xl bg-gradient-to-br from-yellow-200 to-orange-200 flex-shrink-0 flex items-center justify-center overflow-hidden`}>
                      <img src={item.product?.image || 'https://via.placeholder.com/80'} alt={item.product?.title} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="flex justify-between items-start">
                        <h3 className="font-bold text-brown">{item.product?.title}</h3>
                        <button 
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <span className="font-black text-pickle">₹{((item.product?.price || 0) * item.quantity).toFixed(2)}</span>
                      </div>
                    </div>
                  </motion.div>
                ))
