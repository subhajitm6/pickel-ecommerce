...
export function MyOrders() {
  ...
  const filteredAndSortedOrders = useMemo(() => {
    return orders
      .filter(order => {
        const matchesSearch = order.order_number?.toLowerCase().includes(searchQuery.toLowerCase()) || order.id.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesStatus = filterStatus === 'All Orders' || order.order_status?.toLowerCase() === filterStatus.toLowerCase();
                      ...
                      <div className="flex flex-col lg:flex-row justify-between gap-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-4 mb-2">
                            <h3 className="text-2xl font-black text-brown">{order.order_number || order.id}</h3>
                            <span className={`px-4 py-1.5 rounded-full text-sm font-bold border ${getStatusColor(order.order_status)}`}>
                              {order.order_status}
                            </span>
                          </div>
                          <div className="flex flex-col sm:flex-row gap-4 text-brown/60 font-medium">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {/* Address may need a relation fetch, skip for now if unavailable */}
                              Shipping
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-start lg:items-end">
                          ...
                          <div className="text-3xl font-black text-brown mb-4">₹{Number(order.total).toFixed(2)}</div>
                      ...
                      {displayedItems.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex flex-col sm:flex-row gap-6 items-start sm:items-center p-4 rounded-2xl border border-mango/10 hover:border-mango/30 transition-colors bg-white">
                            <div className={`w-24 h-24 rounded-2xl bg-gradient-to-br from-yellow-200 to-orange-200 shrink-0 shadow-inner cursor-pointer flex justify-center items-center overflow-hidden`} onClick={() => fullProduct && navigate(`/products/${fullProduct.slug}`)}>
                               <img src={item.product?.image || 'https://via.placeholder.com/80'} className="w-full h-full object-cover" />
                            </div>
