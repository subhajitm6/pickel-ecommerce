...
export function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setSelectedImage(response.data.image || 'https://via.placeholder.com/150');
      } catch (error) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-2xl font-black text-brown">Loading...</div>;
        ...
        <div className="flex flex-col lg:flex-row gap-12 mb-20">
          ...
          <div className="lg:w-1/2 flex gap-4">
            <div className="flex flex-col gap-4">
              {[(product.image || 'https://via.placeholder.com/150'), ...(product.gallery ? JSON.parse(product.gallery) : [])].filter(Boolean).map((img, i) => (
                <div 
                  key={i} 
          ...
          <div className="lg:w-1/2 flex flex-col justify-center">
            ...
            <h1 className="text-4xl lg:text-5xl font-black text-brown mb-4">{product.title}</h1>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-brown/70 font-bold">{product.rating} ({product.reviews || 0} Reviews)</span>
            </div>
            ...
            <div className="flex flex-col">
              {product.discount_price && (
                <span className="text-2xl font-bold text-brown/40 line-through mb-1">₹{product.price}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-6 text-sm font-bold text-brown/70 mb-8 border-t border-b border-mango/10 py-4">
              <div className="flex items-center gap-2"><Truck className="text-fresh"/> {product.deliveryDays || '3-5 Days'}</div>
          ...
          <div className="min-h-[200px]">
            ...
            {activeTab === 'ingredients' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-wrap gap-4">
                {(product.ingredients ? JSON.parse(product.ingredients) : []).map((ing: any, i: number) => (
            ...
            {activeTab === 'nutrition' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md bg-cream rounded-2xl overflow-hidden border border-mango/20">
                {(product.nutrition ? JSON.parse(product.nutrition) : []).map((item: any, i: number) => (
            ...
            {activeTab === 'reviews' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-10">
                ...
                <p className="text-brown/70 font-bold">Based on {product.reviews || 0} reviews</p>
